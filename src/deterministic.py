...
def deterministic(num, number_trials=200, num_episodes=500, num_iterations=100, max_steps_per_episode=100,
               env1 = 'FrozenLake-v1', env2= 'FrozenLake-v1',
    ...
    for trial in tqdm(range(number_trials)):    
    ...
    np.save('dai/src/results/test/brl_tr_det_online' + str(num) +'.npy', BRL_tr_online) 
    np.save('dai/src/results/test/brl_ts_det_online' + str(num) +'.npy', BRL_ts_online) 
    ...
    np.save('dai/src/results/test/brl_tr_det_' + str(num) +'.npy', BRL_tr) 
